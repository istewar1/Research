...
directory_path = '/Volumes/Ian External HD/Node Data/hdf5files/3x3_test_conversions/'
...
counts_range = {'MUSE01': [100, 1000], 'MUSE04': [100, 1000], \
                'MUSE06': [100, 1000], 'MUSE10': [100, 1000], \
                'MUSE11': [100, 1000], 'MUSE12': [100, 1000]}
...
for i in nodes:
    ...
    #dbFiles = [x for x in dbFiles if (('09-01T' in x) or('09-02T' in x) or('09-03T' in x) or('09-04T' in x) or('09-05T' in x) or('09-06T' in x)or('09-07T' in x) )]
    dbFiles = [x for x in dbFiles if ('09-01T' in x)]

    save_string = '09_01-07'
    ...
    for j in range(1200,86370,1200):
        plt.figure();
        print sum(Node_spectra[i][count:j])
        plt.plot(np.sum(Node_spectra[i][count:j],axis=0));
        plt.yscale('log');
        plt.savefig(outPath+i+'/'+i+'_'+str(count)+'.png')
        plt.close('all')
        count+=j
        ...
        for i in Node_spectra:
            ...
            for k in range(step, len(Node_spectra[i]), step):
                ...
                if False:
                    ...
                    ax.plot(spectra)
                    ax.set_yscale('log')
                    ax.plot(xs, MINOS_analysis.gausswLine(xs, *popt), lw=2, marker='', ls='--', color='k',
                            label=str(popt[3]))
                    ax.plot(xs_th, MINOS_analysis.gausswLine(xs_th, *popt_th), lw=2, marker='', ls='--', color='k',
                            label=str(popt_th[3]))
    ...
    if runAlarms:
        ...
        ksigma_threshold = 10
        sprt_threshold = 1100
        roi_counts, ksigma_alarms, ratio = ksigma_RIO_alg(Node_spectra[i], fore_windows, back_windows, ksigma_threshold,
                                                          'spectra')
        sprt_values, sprt_alarms = sprt_alg(Node_counts[i], Node_livetimes[i], fore_windows, back_windows,
                                            sprt_threshold)
        ...
        if ((len(ksigma_alarms) > 0) or (len(sprt_alarms) > 0)):
            ...
            ax[2].plot(Node_datetimes[i][(len(Node_datetimes[i]) - len(sprt_values))::], np.array(sprt_values),
                       label='SPRT', zorder=10)
