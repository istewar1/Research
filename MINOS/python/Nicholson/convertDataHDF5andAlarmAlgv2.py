...
hdf5outPath='hdf5Files/'
inPath='/rawdata/MINOS/radiationData/'
#inPath='radiationData/'

#list all database files
dbFiles=os.listdir(inPath)
dbFiles=[x for x in dbFiles if 'sqlite3' in x]

#some database files are small and don't contain all tables
#lets remove them from our analysis
dbFiles=np.array([x for x in dbFiles if os.path.getsize(inPath+x) > 1E8])

#sort them by time
posixTimes=[]
zeroTime=dt.datetime.fromtimestamp(0)
for i in range(len(dbFiles)):
    dum=dbFiles[i].split('-')[1:]
    dum='-'.join([str(x) for x in dum]).replace('.sqlite3','')
    posixTimes.append((dt.datetime.strptime(dum,'%Y-%m-%dT%H.%M.%S.%f')-zeroTime).total_seconds())

posixTimes=np.array(posixTimes)
indexs=np.argsort(posixTimes)
dbFiles=dbFiles[indexs]
posixTimes=posixTimes[indexs]

#if there is an HDF5 file, skip
hdf5Files=[x.replace('.hdf5','') for x in os.listdir(hdf5outPath) if '.hdf5' in x and 'algorithmData' not in x and '_ALG' not in x]
hdf5posix=[]
dums=[x.split('MUSE01-')[-1].split('.hdf5')[0] for x in hdf5Files]
hdf5posix=np.array([(dt.datetime.strptime(x,'%Y-%m-%dT%H.%M.%S.%f')-zeroTime).total_seconds() for x in dums])

dum=[]
for i in range(len(dbFiles)):
    if posixTimes[i] not in hdf5posix:
        dum.append(dbFiles[i])

dbFiles=dum

binEdges=np.arange(0,3003,3)
energies=np.linspace(0,3000,1000)

#get energy calibration and calculate non-linearity
energies2x4x16=np.load('eCal_2x4x16.npy')
k40PeakChan=np.abs(energies2x4x16-1460.8).argmin()
slope=k40PeakChan/1460.8
nonLinearity=range(len(energies2x4x16))/linFunc(energies2x4x16,slope)

count = 0  # Use
count1 = 0 # Use
eCal2x4x16=np.linspace(0,3000,2048)

for dbFile in dbFiles:
    ### intialize algorithms
    isotopeList=[{'Isotope':'Total Spectrum','Energies':[0.0],'ROI_Mins':[0.0],'ROI_Maxs':[3500.0],'Fit_Mins':[0.0],'Fit_Maxs':[3500.0],'Sigmas':[0.0]}]
    ...
    sprt=algorithms(foreTimeWindowSize,bkgTimeWindowSize,upperThresholds2x4x16,lowerThresholds2x4x16,kSigmaThresholds,isotopeList)
    kSigma=algorithms(foreTimeWindowSize,bkgTimeWindowSize,upperThresholds2x4x16,lowerThresholds2x4x16,kSigmaThresholds,isotopeList)
    ...
    dataTable='Davis_data'
    if dataTable in tables:
        #
        detDB.getColumn(dataTable,'Time')
        times=np.array(detDB.data)[:,0]
        length=10*(len(times)/10)
        #get the mid point of every 10 elements
        times=np.array([times[i+5] for i in range(0,length,10)])
        #
        detDB.getColumn(dataTable,'Outside_Temperature')
        oTemps=np.array(detDB.data)[:,0]
        #average every 10 elements
        oTemps=np.array([oTemps[i+5] for i in range(0,length,10)])
        #
        detDB.getColumn(dataTable,'Inside_Temperature')
        iTemps=np.array(detDB.data)[:,0]
        #average every 10 elements
        iTemps=np.array([iTemps[i+5] for i in range(0,length,10)])
        #
        detDB.getColumn(dataTable,'Outside_Humidity')
        oHumids=np.array(detDB.data)[:,0]
        #average every 10 elements
        oHumids=np.array([oHumids[i+5] for i in range(0,length,10)])
        #
        detDB.getColumn(dataTable,'Inside_Humidity')
        iHumids=np.array(detDB.data)[:,0]
        #average every 10 elements
        iHumids=np.array([iHumids[i+5] for i in range(0,length,10)])
        #
        detDB.getColumn(dataTable,'Dewpoint')
        dewPoint=np.array(detDB.data)[:,0]
        #average every 10 elements
        dewPoint=np.array([dewPoint[i+5] for i in range(0,length,10)])
        #
        detDB.getColumn(dataTable,'Pressure')
        press=np.array(detDB.data)[:,0]
        #average every 10 elements
        press=np.array([press[i+5] for i in range(0,length,10)])
        #
        detDB.getColumn(dataTable,'Wind_Direction')
        windDir=np.array(detDB.data)[:,0]
        #average every 10 elements
        windDir=np.array([windDir[i+5] for i in range(0,length,10)])
        #
        detDB.getColumn(dataTable,'Wind_Speed')
        windSpd=np.array(detDB.data)[:,0]
        #average every 10 elements
        windSpd=np.array([windSpd[i+5] for i in range(0,length,10)])
        #
        detDB.getColumn(dataTable,'Rain_Rate')
        rain=np.array(detDB.data)[:,0]
        #average every 10 elements
        rain=np.array([rain[i+5] for i in range(0,length,10)])
        #
        detDB.getColumn(dataTable,'Rain_Storm')
        rainStrm=np.array(detDB.data)[:,0]
        #average every 10 elements
        rainStrm=np.array([rainStrm[i+5] for i in range(0,length,10)])
        #
        detDB.getColumn(dataTable,'Solar_Radiation')
        solar=np.array(detDB.data)[:,0]
        #average every 10 elements
        solar=np.array([solar[i+5] for i in range(0,length,10)])

        hdf5write=True
        f=h5py.File(hdf5outPath+outFile,"w")

        dset=f.create_dataset('Weather_Time',(len(times),),dtype='float64', compression="gzip")
        dset[...]=times
        #
        dset=f.create_dataset('Outside_Temperature_F',(len(oTemps),),dtype='f', compression="gzip")
        dset[...]=oTemps
        dset=f.create_dataset('Inside_Temperature_F',(len(iTemps),),dtype='f',compression="gzip")
        dset[...]=iTemps
        #
        dset=f.create_dataset('Outside_Humidity',(len(oHumids),),dtype='f', compression="gzip")
        dset[...]=oHumids
        dset=f.create_dataset('Inside_Humidity',(len(iHumids),),dtype='f',compression="gzip")
        dset[...]=iHumids
        #
        dset=f.create_dataset('Dewpoint_F',(len(dewPoint),),dtype='f', compression="gzip")
        dset[...]=dewPoint
        dset=f.create_dataset('Pressure_inHg',(len(press),),dtype='f',compression="gzip")
        dset[...]=press
        dset=f.create_dataset('Wind_Direction_Deg',(len(windDir),),dtype='f',compression="gzip")
        dset[...]=windDir
        dset=f.create_dataset('Wind_Speed_mph',(len(windSpd),),dtype='f',compression="gzip")
        dset[...]=windSpd
        dset=f.create_dataset('Rain_Rate_inph',(len(rain),),dtype='f',compression="gzip")
        dset[...]=rain
        dset=f.create_dataset('Rain_Storm_in',(len(rainStrm),),dtype='f',compression="gzip")
        dset[...]=rainStrm
        dset=f.create_dataset('Solar_Radiation_wpm2',(len(solar),),dtype='f',compression="gzip")
        dset[...]=solar
        
    else:
        count+=1
        f3 = open("logs/No_Davis.txt","a+")
        f3.write("Data file [%d]: %s\n" %(count,dbFile))
        f3.close()

    dataTable='Osprey_2x4x16_data'
    if dataTable in tables:        
        ...
        for i in range(len(spectra)):
            ...
            if np.around(np.sum(windowLiveTime) > windowSize):
                ...
                eCal=doEcal(popt[3],energies2x4x16,nonLinearity)
                windowSpec=[]
                windowLiveTime=[]
                if icount == 0:
                    icount=icount+1
            ...
            if icount == 0:
                ...
                dum=[]
                for k in range(len(isotopeList)):
                    indexs=np.where( (energies2x4x16 >= isotopeList[k]['ROI_Mins'][0]) & (energies2x4x16 <= isotopeList[k]['ROI_Maxs'][0]) )[0]
                    dum.append(np.sum(spectrum[indexs])/liveTime)
                countRates.append(dum)
                sprtMetrics.append([0])
                kSigmaMetrics.append([0])
                ###
                # start algorithms
                ###
                ...
                # Run algorithms
                ###
                sprt.doSPRT(liveTime,energies,rebinedSpectrum)
                kSigma.dokSigma(liveTime,energies,rebinedSpectrum)
                if sprt.alarm or kSigma.alarm:
                    alarm=True
                else:
                    alarm=False
                alarms.append(alarm)
                sprtMetrics.append([x for x in sprt.metrics])
                kSigmaMetrics.append([x for x in kSigma.metrics])
                if sprt.alarm and kSigma.alarm:
                    alarmSpec=sprt.aggSpec
                    alarmLiveTime=sprt.aggLiveTime
                    if alarmStartTime == 0:
                        alarmStartTime=time
                        alarmStartIndex=i
                elif sprt.alarm and not kSigma.alarm:
                    alarmSpec=sprt.aggSpec
                    alarmLiveTime=sprt.aggLiveTime
                    if alarmStartTime == 0:
                        alarmStartTime=time
                        alarmStartIndex=i
                elif kSigma.alarm and not sprt.alarm:
                    alarmSpec=kSigma.aggSpec
                    alarmLiveTime=kSigma.aggLiveTime
                    if alarmStartTime == 0:
                        alarmStartTime=time
                        alarmStartIndex=i
                else:
                    if len(alarmSpec) > 0:
                        alarmSpectra.append(alarmSpec)
                        alarmLiveTimes.append(alarmLiveTime)
                        alarmTimes.append([alarmStartTime,time])
                        alarmIndexs.append([alarmStartIndex,i-1])
                        alarmStartTime=0
                        alarmSpec=[]
                dum=[]
                for k in range(len(isotopeList)):
                    indexs=np.where( (energies >= isotopeList[k]['ROI_Mins'][0]) & (energies <= isotopeList[k]['ROI_Maxs'][0]) )[0]
                    dum.append(np.sum(rebinedSpectrum[indexs])/liveTime)
                countRates.append(dum)
        ...
        dset=f.create_dataset('2x4x16Spectra',(len(rebinedSpectra),len(rebinedSpectra[0])),dtype='f', compression="gzip")
        dset[...]=rebinedSpectra
        dset=f.create_dataset('2x4x16LiveTimes',(len(liveTimes),),dtype='f', compression="gzip")
        dset[...]=liveTimes
        dset=f.create_dataset('2x4x16Times',(len(times),),dtype='float64',compression="gzip")
        dset[...]=times
        fileName=dbFile.replace('.sqlite3','_peakLocs.npy')
        np.save('logs/'+fileName,peakLocs)

        #write algorithm results
        outFileHDF=dbFile.replace('.sqlite3','_ALG.hdf5')
        outFileHDF='MUSE01-'+'-'.join(outFileHDF.split('-')[1:])
        countRates=np.array(countRates)
        sprtMetrics=np.array(sprtMetrics)
        kSigmaMetrics=np.array(kSigmaMetrics)
        allTimes=np.array(allTimes)
        fff=h5py.File(hdf5outPath+outFileHDF,"w")
        dset=fff.create_dataset('sprtMetrics',(len(sprtMetrics),len(sprtMetrics[0])),dtype='f', compression="gzip")
        dset[...]=sprtMetrics
        dset=fff.create_dataset('kSigmaMetrics',(len(kSigmaMetrics),len(kSigmaMetrics[0])),dtype='f', compression="gzip")
        dset[...]=kSigmaMetrics
        dset=fff.create_dataset('countRates',(len(countRates),len(countRates[0])),dtype='f', compression="gzip")
        dset[...]=countRates
        dset=fff.create_dataset('times',(len(allTimes),),dtype='float64', compression="gzip")
        dset[...]=allTimes
        if len(alarmSpectra) > 0:
            dset=fff.create_dataset('alarmSpectra',(len(alarmSpectra),len(alarmSpectra[0])),dtype='f',compression="gzip")
            dset[...]=alarmSpectra
            dset=fff.create_dataset('alarmLiveTimes',(len(alarmLiveTimes),),dtype='f',compression="gzip")
            dset[...]=alarmLiveTimes
            dset=fff.create_dataset('alarmTimes',(len(alarmTimes),len(alarmTimes[0])),dtype='float64',compression="gzip")
            dset[...]=alarmTimes
            dset=fff.create_dataset('alarmIndexs',(len(alarmIndexs),len(alarmIndexs[0])),dtype='i8',compression="gzip")
            dset[...]=alarmIndexs
        else:
            dset=fff.create_dataset('alarmSpectra',(0,0),dtype='f',compression="gzip")
            dset=fff.create_dataset('alarmLiveTimes',(0,),dtype='f',compression="gzip")
            dset=fff.create_dataset('alarmTimes',(0,0),dtype='float64',compression="gzip")
            dset=fff.create_dataset('alarmIndexs',(0,0),dtype='i8',compression="gzip")
        fff.close()
        
    else:
        f2 = open("logs/No_2x4x16.txt","a+")
        f2.write("Data file: %s\n" %(dbFile))
        f2.close()
    
    #because a lot of the 3x3 data is bad, lets just leave it out for now
    #just get neutrons
    dataTable='Osprey_3x3_data'
    if dataTable in tables:
        detDB.getHeader(dataTable)
        header=np.array(detDB.header)
        #
        ...
        indexs=np.where(liveTimes < 0.05)[0]
        if len(indexs) > 0:
            f4 = open('logs/liveTimeIssues.txt','a+')
            for index in indexs:
                f4.write('3x3,%.6e\n' % liveTimes[index])
            f4.close()
        length=10*(len(liveTimes)/10)
        ...
        times=np.array([times[i+5] for i in range(0,length,10)])
        #
        detDB.getColumn(dataTable,'TTL_Counts')
        neutronCnts=np.array(detDB.data)[:,0]
        neutronCnts=np.add.reduceat(neutronCnts[:length],np.arange(0,length,10))

        if not hdf5write:
            f=h5py.File(hdf5outPath+outFile,"w")
            hdf5write=True

        dset=f.create_dataset('neutronCnts',(len(neutronCnts),),dtype='f', compression="gzip")
        dset[...]=neutronCnts
        dset=f.create_dataset('neutronLiveTimes',(len(liveTimes),),dtype='f', compression="gzip")
        dset[...]=liveTimes
        dset=f.create_dataset('neutronTimes',(len(times),),dtype='float64',compression="gzip")
        dset[...]=times
    else:
        count+=1
        f3 = open("logs/No_3x3.txt","a+")
        f3.write("Data file [%d]: %s\n" %(count,dbFile))
        f3.close()
